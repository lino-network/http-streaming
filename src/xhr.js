  ...
  if (!error && reqResponse) {
    ...
    if (!request.bandwidth) {
      request.bandwidth = Math.floor(
        (request.bytesReceived / request.roundTripTime) * 8 * 1000,
      );
    }
  }
 ...
 */
const callAllRequestHooks = async (requestSet, options) => {
  ...
  for (const requestCallback of requestSet) {
    newOptions = await requestCallback(newOptions);
  }
 ...
 */
const callAllResponseHooks = async (responseSet, request, error, response) => {
  if (!responseSet || !responseSet.size) {
    return;
  }
  for (const responseCallback of responseSet) {
    await responseCallback(request, error, response);
  }
};
...
const xhrFactory = function () {
  const xhr = async function XhrFunction(options, callback) {
    ...
    const beforeRequest =
      XhrFunction.beforeRequest || videojs.Vhs.xhr.beforeRequest;
    ...
    const _requestCallbackSet =
      XhrFunction._requestCallbackSet ||
      videojs.Vhs.xhr._requestCallbackSet ||
      new Set();
    const _responseCallbackSet =
      XhrFunction._responseCallbackSet || videojs.Vhs.xhr._responseCallbackSet;
    ...
    const beforeRequestOptions = await callAllRequestHooks(
      _requestCallbackSet,
      options,
    );
    ...
    const request = xhrMethod(
      beforeRequestOptions || options,
      async function (error, response) {
        // call all registered onResponse hooks
        await callAllResponseHooks(
